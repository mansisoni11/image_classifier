...
class App:
    def __init__(self, window=tk.Tk(), window_title = "Camera_classifier"):
        self.window = window
    ...
    def save_for_class(self, class_num):
        ...
        if not os.path.exists('1'):
            os.mkdir('1')
        if not os.path.exist('2'):
            os.mkdir('2')        

        cv.imwrite(f'{class_num}/frame{self.counters[class_num-1]}.jpeg',cv.cvtColor(frame, cv.COLOR_RGB2GRAY))
        img = PIL.Image.open(f'{class_num}/frame{self.counters[class_num-1]}.jpeg')
        img.thumbnail((150,150),PIL.Image.ANTIALIAS)
        img.save(f'{class_num}/frame{self.counters[class_num-1]}.jpeg')
    ...
    def update(self):
        ...
        if ret:
            ...
            self.canva.create_image(0,0,image= self.photo,anchor=tk.NW)
